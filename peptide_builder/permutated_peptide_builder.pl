#!/home/ivan/bin/perl
# usage: ./peptide_builder.pl <SEQUENCE>
#
...
my @aminokwasy=('A','C','D','E','F','G','H','I','K','L','M','N','P','Q','R','S','T','V','W','Y');
foreach my $ami1(@aminokwasy){
  foreach my $ami2(@aminokwasy){
	my $pept=$ami1.$ami2;
	open(OUT,">$pept.mol");	
	my $handle=select(OUT);
	#my $chain = build_sequence(shift);
	my $chain = build_sequence($pept);
